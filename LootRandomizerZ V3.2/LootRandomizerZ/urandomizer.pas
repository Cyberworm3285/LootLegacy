  ...
  begin
   ...
   For j:=0 to templist.Count-1 Do
    begin
     ...
     IF (a) and (b) THEN Errors.Append('I. ist Ruestung und Waffe!(E12)');
     IF (((a) or (b)) and not (e)) THEN Errors.Append('[Warnung] Deklariert, kein Wert!(W00)');
     IF (self.Gebiete.Count=0) THEN Errors.Append('[Warnung] Gebietsdatei ist leer!(W01)');
     //IF (self.Attribute)
     ...
     For K:=1 to max Do
      begin
       Case clearString[k] of
        '(':If (K<>1) THEN If not (clearstring[k+1] in ['1'..'9']) THEN Errors.Append('Ungültige Anzahl-Konfig!(E00)('+clearstring[k+1]+')');
        ',':
         begin
          If (K<>1) THEN If not (clearstring[k-1] in ['0'..'9']) THEN Errors.Append('Ungültige Anzahl-Konfig!(E01)('+clearstring[k-1]+')');
          If (K<>max) THEN If not (clearstring[k+1] in ['1'..'9']) THEN Errors.Append('Ungültige Anzahl-Konfig!(E02)('+clearstring[k+1]+')');
         end;
        ')':IF (K<>1) THEN If not (clearstring[k-1] in ['1'..'9']) THEN Errors.Append('Ungültige Anzahl-Konfig!(E03)('+clearstring[k-1]+')');
        '{':If (K<>max) THEN If not (clearstring[k+1] in ['0'..'9']) THEN Errors.Append('Ungültige Wert-Konfig!(E04)('+clearstring[k+1]+')');
        '-':
         begin
          If (K<>1) THEN If not (clearstring[k-1] in ['0'..'9']) THEN Errors.Append('Ungültige Wert-Konfig!(E05)('+clearstring[k-1]+')');
          If (K<>max) THEN If not (clearstring[k+1] in ['1'..'9']) THEN Errors.Append('Ungültige Wert-Konfig!(E06)('+clearstring[k+1]+')');
         end;
        '}':If (K<>1) THEN If not (clearstring[k-1] in ['0'..'9']) THEN Errors.Append('Ungültige Wert-Konfig!(E07)('+clearstring[k-1]+')');
        '#':If (K<>max) THEN If not (clearstring[k+1] in ['1'..'9']) THEN Errors.Append('Ungültige Attribut-Konfig!(E08)('+clearstring[k+1]+')');
        '\':
         begin
          If (K<>max)
           THEN
            begin
             If not (clearstring[k+1] in ['1'..'9'])
              THEN Errors.Append('Ungültige Gebiets-Konfig!(E09)('+clearstring[k+1]+')')
              ELSE IF (StrToInt(clearstring[k+1])>Gebiete.Count)
               THEN Errors.Append('Ungültige Gebiets-Konfig!(E10)('+clearstring[k+1]+')')
               ELSE IF (StrToInt(clearstring[k+1])<1)
                THEN Errors.Append('Ungültige Gebiets-Konfig!(E11)('+clearstring[k+1]+')')
            end;
         end;
        end;
      end;
...
begin
 ...
 IF (self.LootCount.Count=self.Sources.Count-1)
  THEN
   begin
    show('Ungültige Lootquellen Konfig. !(E13)');
    inc(errorCount);
   end;
 IF (self.NItems.Count=0) THEN show('[Warnung] NItems ist leer(W02)');
 IF (self.RItems.Count=0) THEN show('[Warnung] NItems ist leer(W03)');
 IF (self.VRItems.Count=0) THEN show('[Warnung] NItems ist leer(W04)');
 IF (self.LItems.Count=0) THEN show('[Warnung] NItems ist leer(W05)');
 IF (self.NItems.Count +
  ...
  THEN
   begin
    show('Keine Items geladen!(E14)');
    inc(errorCount);
   end;
 IF (self.Attribute.Count=0) THEN show('[]');
