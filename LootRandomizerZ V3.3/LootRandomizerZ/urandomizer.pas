...
type
  ...
  private
    Rarities,EffectW,EffectA,Sources,Attribute,LootCount,Gebiete: TSTringlist;
  ...
  public
    ...
    Function getValue(rarity:integer;Item:String):String            ;
...
begin
 ...
 For i:=1 to rdm Do
  begin
   ...
   IF (e) THEN LValue:=self.getValue(LRarity,LTypeN);
     ...
     begin
      ...
      CASE MetaType of
       0:
        begin
         IF (LValue<>'')
          THEN Loot:=Loot+LTypeN+'|'+LRarityN+'|Wert:'+LValue+Attribut+Count+'|'
          ELSE Loot:=Loot+LTypeN+'|'+LRarityN+Attribut+Count+'|';
        end;
       1: Loot:=Loot+LTypeN+'|'+LRarityN+'|'+LValue+'Dmg'+'|'+IntToStr(random(5)+1)+' Parade'+ Attribut+Count+'|';
       2: Loot:=Loot+LTypeN+'|'+LRarityN+'|'+LValue+'DmgReduction'+ Attribut+Count+'|';
      end;
     end
    ELSE
     begin
    ...
    THEN
     begin
      ...
      IF (b)
       THEN
        begin
         IF (self.EffectA.Count<>0)
          THEN self.Memo1.Append(EffectA[random(self.EffectA.Count)])
          ELSE show('Kein Effekt verfügbar!');
        end
       ELSE IF (a)
        THEN
         begin
          IF (self.EffectW.Count<>0)
           THEN self.Memo1.Append(EffectW[random(self.EffectW.Count)])
           ELSE show('Kein Effekt verfügbar!');
         end;
     end
    ELSE
...
begin
 ...
 self.Gebiete:=TStringlist.Create       ;
 ...
 self.Gebiete.LoadFromFile('..\lootrandomizerZ\config\Gebiete.txt')                   ;
 end;
 Entry:=self.Rarities.Count  ;
 ...
 show('Es wurden ' + IntToStr(self.Gebiete.Count)+' Gebiete gefunden!');
 source:=self.Sources.Count;
 IF (self.Sources.Count<>0) THEN lBound:=StrToInt(self.Sources[self.Sources.Count-1]);
 rBound:=LBound+source-2;
 errorCount:=0;
 IF (self.LootCount.Count<>self.Sources.Count-1)
  THEN
   begin
    show('[Error]Ungültige Lootquellen Konfig. !(E13)');
    inc(errorCount);
   end;
 IF (self.Sources.Count<2)
  THEN
   begin
    show('[Error]Keine Quellen angegeben!(E14)');
    inc(errorCount);
   end;
 IF (self.Rarities.Count<5)
  THEN
   begin
    show('[Error]Ungültige Seltenheiten(E15)');
   end;
 IF (self.NItems.Count +
     self.RItems.Count +
     self.VRItems.Count+
     self.LItems.Count  =0)
  THEN
   begin
    show('[Error]Keine Items geladen!(E16)');
    inc(errorCount);
   end;
 IF (self.Gebiete.Count=0) THEN show('[Warnung] Gebietsdatei ist leer!(W01)');
 IF (self.NItems.Count=0) THEN show('[Warnung] NItems ist leer(W02)');
 IF (self.RItems.Count=0) THEN show('[Warnung] RItems ist leer(W03)');
 IF (self.VRItems.Count=0) THEN show('[Warnung] VRItems ist leer(W04)');
 IF (self.LItems.Count=0) THEN show('[Warnung] LItems ist leer(W05)');
 IF (self.Attribute.Count=0) THEN show('[Warnung] Keine Attribute(W06)');
 IF (self.Rarities.Count<>5) THEN show('[Warnung] Ungültige Seltenheiten(W07)');
 IF (self.EffectA.Count=0) THEN show('[Warnung] Keine L.A.Effekte(W08)');
 IF (self.EffectW.Count=0) THEN show('[Warnung] Keine L.W.Effekte(W09)');
 IF (errorCount=0)
  THEN
   begin
  ...
  ELSE
   begin
    ...
    self.Shape1.Brush.Color:=clRed;
    show('Initialisierung fehlgeschlagen!');
    show(intToStr(errorCount)+' Errors');
    exit;
   end;
  ...
  begin
   ...
   For j:=0 to templist.Count-1 Do
    begin
     ...
     IF (a) and (b) THEN Errors.Append('[Error]I. ist Ruestung und Waffe!(E12)');
     IF (((a) or (b)) and not (e)) THEN show('[Warnung] Deklariert, kein Wert!(W00)');
     //IF (self.Attribute)
     ...
     For K:=1 to max Do
      begin
       Case clearString[k] of
        '(':If (K<>1) THEN If not (clearstring[k+1] in ['1'..'9']) THEN Errors.Append('[Error]Ungültige Anzahl-Konfig!(E00)('+clearstring[k+1]+')');
        ',':
         begin
          If (K<>1) THEN If not (clearstring[k-1] in ['0'..'9']) THEN Errors.Append('[Error]Ungültige Anzahl-Konfig!(E01)('+clearstring[k-1]+')');
          If (K<>max) THEN If not (clearstring[k+1] in ['1'..'9']) THEN Errors.Append('[Error]Ungültige Anzahl-Konfig!(E02)('+clearstring[k+1]+')');
         end;
        ')':IF (K<>1) THEN If not (clearstring[k-1] in ['1'..'9']) THEN Errors.Append('[Error]Ungültige Anzahl-Konfig!(E03)('+clearstring[k-1]+')');
        '{':If (K<>max) THEN If not (clearstring[k+1] in ['0'..'9']) THEN Errors.Append('[Error]Ungültige Wert-Konfig!(E04)('+clearstring[k+1]+')');
        '-':
         begin
          If (K<>1) THEN If not (clearstring[k-1] in ['0'..'9']) THEN Errors.Append('[Error]Ungültige Wert-Konfig!(E05)('+clearstring[k-1]+')');
          If (K<>max) THEN If not (clearstring[k+1] in ['1'..'9']) THEN Errors.Append('[Error]Ungültige Wert-Konfig!(E06)('+clearstring[k+1]+')');
         end;
        '}':If (K<>1) THEN If not (clearstring[k-1] in ['0'..'9']) THEN Errors.Append('[Error]Ungültige Wert-Konfig!(E07)('+clearstring[k-1]+')');
        '#':If (K<>max) THEN If not (clearstring[k+1] in ['1'..'9']) THEN Errors.Append('[Error]Ungültige Attribut-Konfig!(E08)('+clearstring[k+1]+')');
        '\':
         begin
          If (K<>max)
           THEN
            begin
             If not (clearstring[k+1] in ['1'..'9'])
              THEN Errors.Append('[Error]Ungültige Gebiets-Konfig!(E09)('+clearstring[k+1]+')')
              ELSE IF (StrToInt(clearstring[k+1])>Gebiete.Count)
               THEN Errors.Append('[Error]Ungültige Gebiets-Konfig!(E10)('+clearstring[k+1]+')')
               ELSE IF (StrToInt(clearstring[k+1])<1)
                THEN Errors.Append('[Error]Ungültige Gebiets-Konfig!(E11)('+clearstring[k+1]+')')
            end;
         end;
        end;
      end;
     IF(Errors.Count<>0)
      THEN
       begin
        show('>>>>>>>X<<<<<<<<');
        show('Bei Item '+self.clearName(templist[j])+' gibt es Fehler('+intToStr(Errors.Count)+'):');
        ...
        show('[Item gelöscht!]');
        Case i of
         1: self.NItems.Strings[j]:='ITEM GELÖSCHT';
         2: self.RItems.Strings[j]:='ITEM GELÖSCHT';
         3: self.VRItems.Strings[j]:='ITEM GELÖSCHT';
         4: self.LItems.Strings[j]:='ITEM GELÖSCHT';
        end;
        show('<<<<<<<X>>>>>>>>');
       end
      ELSE
...
var
 i,j,k,c:integer;
 TempA,TempB,TempC,FixedA,FixedB:Boolean;
 Key:Char;
 templist:TStringlist;
begin
 IF not (self.initialised)
 ...
 show('----------------');
 FixedA:=(Checkbox1.Checked) or (Checkbox2.Checked) or (Checkbox4.Checked);
 IF (FixedA)
  THEN
   begin
  ...
  begin
   ...
   For i:=1 to templist.Count Do
    begin
     ...
     tempC:=false;
     fixedB:=false;
     For j:=1 to length(templist[i-1]) Do
      begin
       IF (self.Censor(templist[i-1])[j]=Key) and (Key<>'_') THEN TempA:=True;
       IF (j<>length(templist[i-1])) and (self.Censor(templist[i-1])[j]='\')
        THEN
         begin
          fixedB:=true;
          IF (strToInt((self.Censor(templist[i-1])[j+1]))=self.convertStr(self.Edit1.Text,0)) or
             (self.Edit1.Text='-')
           THEN
            begin
             tempB:=true
       ...
       IF (Key='_')
        THEN
         begin
          IF (self.Censor(templist[i-1])[j] in ['~','$']) THEN tempC:=true;
         end;
      end;
     If (Key='_') and (TempC) THEN TempA:=false ELSE tempA:=true;
     IF ((TempB or not fixedB) and (TempA or not fixedA))
      THEN
       begin
        show(IntToStr(c)+': '+templist[i-1]);
        inc(c);
       end;
    end;
  end;
...
begin
  ...
  Bound:=(self.Gebiete.Count);
  self.Edit1.Text:=intToStr(random(Bound)+1);
  self.Edit3.Text:='-';
  self.Edit4.Text:='-';
  self.Edit5.Text:='-';
  self.Edit6.Text:='-';
...
begin
 ...
 show('--->X<----');
 show('Gebiete:');
 for i:=0 to self.Gebiete.Count-1 do
  begin
   show(IntToStr(i+1)+':'+self.Gebiete[i]);
  end;
end;
           ...
           For i:=0 to templist.Count-1 Do
            begin
             ...
             For j:=1 to length(templist[i]) Do
              begin
               IF self.Censor(templist[i])[j] in ['~','$']
                THEN fixed:=true
              end;
             If not (fixed) THEN Filter.Append(templist[i]);
            end;
          end;
       IF (Key<>'_')
        THEN
...
Function TForm1.getValue(rarity:integer;Item:String):String;
var
 Ug,Og,rdm:Integer;
 E:String;
 begin
...
temp,Attribut:String;
 begin
  ...
  IF (self.Attribute.Count=0)
   THEN
    begin
     show('keine Attribute verfügbar!');
     exit;
    end;
  IF (i=1)
...
begin
 ...
 IF(length(E)<1)
  ...
  ELSE
   begin
    IF (raw[1]='-') THEN result:=-1*strToInt(E)
                    ELSE result:=StrToInt(E);
   end;
 IF (E<>raw)
