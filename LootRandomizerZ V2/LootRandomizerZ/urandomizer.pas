...
type
  ...
  private
    Stages,Normal,Rare,VRare,Legendary,Rarities,EffectW,EffectA,Sources,Attribute: TSTringlist;
...
begin
 ...
 show(LtypeN+' -> '+copy);
 copyClean:=self.Censor(LTypeN);
 IF (copyClean<>LTypeN)
  THEN
   begin
    show('filtere Sonderzeichen..');
    show(LTypeN+' -> '+copyClean);
   end;
 self.setMetaType(copyClean,a,b,c,d);
 IF (a) and (b)
  THEN
   begin
...
i,c:integer;
  begin
   ...
   For i:=0 To self.Sources.Count-2 Do
    begin
     show(IntToStr(c)+': '+self.Sources[i]);
     inc(c);
    end;
 end;
...
temp,Attribut:String;
 begin
  ...
  For j:=1 to i Do
   begin
    rdm:=random(self.Attribute.Count+self.lBound)+AMP;
    IF (rdm<0) THEN rdm:=0;
